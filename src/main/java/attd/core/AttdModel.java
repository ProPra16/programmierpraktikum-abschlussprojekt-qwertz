...
public class AttdModel {
	...
	private void holdCode(State state, boolean alert) {
		...
		if (alert && showAlert(new Alert(AlertType.CONFIRMATION, "Mˆchten Sie die ƒnderungen am Code speichern?", ButtonType.OK))) {
	...
	public void check() {
		...
		if (currentVersion != null) {
			...
			try {
				switch (state) {
				case writeFailingTest:
					if (compiler.testCompilable(inc)) { // pr√ºft ob der test
														// kompiliert und es
				...
				case makeTheTestPass:
					...
					if (compiler.passTests(inc, test)) { // pr√ºft ob der Code
															// die Tests besteht
															// && der code
															// kompiliert
						stopTimeline();
					...
					break;
				case refactor:
					if (compiler.passTests(inc, test)) { // pr√ºft ob refract
															// kompiliert &&
															// tests besteht
						holdCode(state,true);
						if (exercise.getConfigurations().AcceptanceTestEnabled()
								&& compiler.passTests(inc, currentVersion.getCodes().getAcceptanceCode().getCode())) {// pr√ºft
																														// ob
					...
					break;
				case acceptanceTest:
					if (compiler.testCompilable(inc)) { // pr√ºft ob der
														// akzeptanztest
														// kompiliert && es
	...
	private void setErrors(List<CompileError> errors) {
		errorProperty.set(FXCollections.observableList(errors));
	}
	...
	public void init(Version version, Exercise exercise) {
		if (oldVersion != null
				&& showAlert(new Alert(AlertType.WARNING, "Mˆchten Sie die ƒnderungen an der Aufgabe speichern?", ButtonType.YES, ButtonType.NO))) {
			catalogRepository.save();
		} else {
			currentVersion = oldVersion;
		}
		if (showAlert(new Alert(AlertType.CONFIRMATION, "Mˆchten Sie die Aufgabe starten?", ButtonType.YES, ButtonType.NO))) {
